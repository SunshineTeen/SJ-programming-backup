...
int chk_stu_no(char *stu_no, char *check_str, const char *prompt)
{
	...
	for (i=0; i<7; i++)
	    if (check_str[i]<'0' || check_str[i]>'9')  //不是 '0'-'9'之间的字符
	    	break;
	if (i<7) {
	    cout << prompt << "不是7位数字" << endl;
	    return -1;
	    }
...
int main(int argc, char **argv)
{
    ...
    /* 带的第1个参数：要检查的学号 */
    ret = chk_stu_no(stu_no_1, argv[1], "要检查的学号");
    if (ret < 0)
	return -1;

    /* 带的第2个参数：要匹配的学号 */
    ret = chk_stu_no(stu_no_2, argv[2], "要匹配的学号");
    if (ret < 0)
	return -1;
